...
class QNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.layers = torch.nn.Sequential(
            # Final input layout:
            # 0-79 current state, 80-319 opponent-turn history,
            # 320-396 candidate move.
            torch.nn.Linear(397, 128),
            torch.nn.ReLU(),
...
class QLearningBotTrackMoves(Bot):
    ...
    def encode_observation(self, observation: dict):
        ...
        # In the full state, the oldest, middle, and newest turns occupy
        # 80-159, 160-239, and 240-319 respectively.
        last_3_moves = []
        ...
        turns = list(move_history)[-3:]
        turns = [None] * (3 - len(turns)) + turns

        for turn in turns:
            encoded_turn = [0.0] * 80

            if turn is None:
                last_3_moves.extend(encoded_turn)
                continue
    ...
    def encode_move(
        ...
        if isinstance(move[0], str):
            ...
            encoded_move = (
                # Candidate 0-53; combined input 320-373.
                    remaining_hand
                    # Candidate 54; combined input 374.
                    + [new_hand_count]
                    # Candidate 55-69; combined input 375-389.
                    + final_card_encoding
                    # Candidate 70-73; combined input 390-393.
                    + resulting_color_encoding
                    # Candidate 74-76; combined input 394-396.
                    + non_card_encoding
            )
            return encoded_move
